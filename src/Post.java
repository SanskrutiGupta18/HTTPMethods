...
public class Post {
	...
	public static void main(String[] args) {
		...
		String reqbody="{\r\n"
				...
				+ "    \"dept\": \"sales\"\r\n"
				+ "}";
		int statuscode=given().header("Content-Type","application/json").body(reqbody).when().post("/api/users").then().extract().statusCode();
		System.out.println("Status Code:"+statuscode);
		
		String resbody=given().header("Content-Type","application/json").body(reqbody).when().post("/api/users").then().extract().response().asString();
		...
		String id_res = jspres.getString("id");
		String createdAt_res = jspres.getString("createdAt").substring(0,10);
		
		//Validation
		Assert.assertEquals(name_req,name_res);
		Assert.assertEquals(job_req,job_res);
		Assert.assertEquals(dept_req,dept_res);
		Assert.assertNotNull(id_res);
		Assert.assertEquals(expecteddate,createdAt_res);	
