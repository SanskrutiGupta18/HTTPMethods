...
RestAssured.baseURI="https://reqres.in";
		...
		String reqbody="{\r\n"
				...
				+ "    \"dept\": \"sales\"\r\n"
				+ "}";
		int statuscode=given().header("Content-Type","application/json").body(reqbody).when().put("/api/users/2").then().extract().statusCode();
		System.out.println("Status Code:"+statuscode);
		
		String resbody=given().header("Content-Type","application/json").body(reqbody).when().put("/api/users/2").then().extract().response().asString();
